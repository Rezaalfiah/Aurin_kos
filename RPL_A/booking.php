    ...
    <div class="container">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                $check_query = "SELECT COUNT(*) FROM registrasi WHERE email = ?";
                $check_stmt = mysqli_prepare($DBConnect, $check_query);
                if ($check_stmt) {
                    ...
                    if ($count == 0) {
                        ...
                        // Validate KTP file extension and size, then upload and insert into database
                        ...
                        if (!in_array($ktp_extension, $allowed_extensions)) {
                            ...
                            // Define the upload directory for KTP file
                            $upload_dir = "uploads/";

                            // Define a unique name for the KTP file
                            $ktp_new_name = uniqid() . "_" . basename($ktp_name);

                            // Move the KTP file to the upload directory
                            $ktp_path = $upload_dir . $ktp_new_name;
                            if (!move_uploaded_file($ktp_tmp, $ktp_path)) {
                                $error = "Gagal mengunggah file KTP.";
                            } else {
                                // Insert data into the data_diri table
